...
function draw() {
  ...
  if (displayRuler) {
    if (rulerOrigin == -1) {
      ...
      text(
        "Distance: " + distance,
        rulerOrigin.x + (dx / 2) * (mouseX > rulerOrigin.x ? 1 : -1) - 50,
        10 + rulerOrigin.y + (dy / 2) * (mouseY > rulerOrigin.y ? 1 : -1)
      );
    }
  }

  document.getElementById("remove-unit").style.backgroundColor = removalEnabled
    ? "#4e685a"
    : "#6b8a7a";
  ...
  document.getElementById("mask_btn").style.backgroundColor = maskDrawingEnabled
    ? "#4e685a"
    : "#6b8a7a";
...
function startDrawing() {
  ...
  if (selectedUnit === "mm") {
    ...
    units = (parseInt(feetInput.value) * 12 + parseInt(inchInput.value)) * 25.4;
  }
    ...
    for (let y = minY; y < maxY; ) {
      ...
      if (index >= 0) {
        removalPoints[index].forEach((point) => {
          if (
            x < point.x &&
            x + smallRectWidth * !orient + smallRectHeight * orient > point.x &&
            y < point.y &&
            y + smallRectHeight * !orient + smallRectWidth * orient > point.y
          ) {
            flag = false;
      ...
      if (
        x + smallRectWidth * !orient + smallRectHeight * orient <= maxX &&
        y + smallRectWidth * orient + smallRectHeight * !orient <= maxY &&
        flag
      ) {
